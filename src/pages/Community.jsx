...
export default function Community() {
  ...
  return (<>
    ...
    <section className="section"><div className="container"><div style={{textAlign:"center"}}><div className="label">Connect With Us</div><h2 className="section-title">Many Ways to <em>Connect</em></h2></div>
      <div className="com-grid">{ways.map(w=><div key={w.title} className="com-card"><div style={{fontSize:40,marginBottom:16}}>{w.icon}</div><h3 style={{fontSize:20,marginBottom:10}}>{w.title}</h3><p style={{fontSize:14,color:"var(--grey)",lineHeight:1.7,marginBottom:24}}>{w.desc}</p><button className="com-btn" style={{background:w.color}}>{w.action}</button></div>)}</div>
    </div></section>
